...
def lfsr(initial_vector, poly, m):
    ...
    for i in range(num_clk):
        if i == 0:
            ...
            key.append(k)
        print(f"{i + 1}\t\t{shifted_vector}")
    return key


def encryption(plain_txt_):
    plain_txt_ = txt_to_ascii(plain_txt_)
    i_vector = input("Enter the initial vector (e.g., 1 0 0): ").split()
    i_vector = [int(bit) for bit in i_vector]
    poly = input("Enter the polynomial (e.g., 1 2): ").split()
    poly = [int(factor) for factor in poly]
    mask = int(input("What is number of flip flop(e.g., 3): "))
    key = lfsr(i_vector, poly, mask)
...
decrypted_txt = decryption(encrypted_txt)
